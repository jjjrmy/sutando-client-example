  ...
  <div class="container mx-auto p-4">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      <h2 class="text-xl font-bold mb-4">
        Posts
        <small>{{
          user.has_edited_posts ? "has edited posts" : "no edited posts"
        }}</small>
      </h2>
      <div class="space-y-4">
        <div
          ...
          :key="post.id"
          class="flex space-x-4 items-center p-4 border rounded"
        >
          <NuxtLink :to="`/post/${post.id}`">{{ post.title }}</NuxtLink>
          <div class="text-sm text-gray-600">
            {{ post.was_edited ? "edited" : "not edited" }}
          </div>
...
function removePost(id: number) {
  user.value.setRelation(
    "posts",
    user.value.posts.filter((post: Post) => post.id !== id)
  );
}
</script>
